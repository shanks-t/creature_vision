...
@dsl.component(
    base_image="python:3.10", packages_to_install=["google-api-python-client"]
)
...
def creature_vision_pipeline(
    ...
    wait_task = wait_for_dataflow_job(
        project_id=project_id, region=region, job_name=df_job_name
    ).after(dataflow_task)
...
compiler.Compiler().compile(
    pipeline_func=creature_vision_pipeline, package_path="creature_vision_pipeline.json"
)
